...
public interface MemberRepository extends CrudRepository<Member, Integer> {
    ...
    @Query(value = "select member.handle, member.rating, member.solved_class, count(distinct solve.problem_id) as solve_count, member.rank from member join solve on solve.id = member.id group by member.id", nativeQuery = true)
